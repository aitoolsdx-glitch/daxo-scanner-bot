...
async def call_stats(callback: types.CallbackQuery):
    await callback.message.answer(f"📈 Уникальных пользователей: {len(users_db)}")
...
@dp.message()
async def analyze_content(message: types.Message):
    ...
    try:
        # ЗАМЕНА МОДЕЛИ НА ПОДДЕРЖИВАЕМУЮ
        completion = client.chat.completions.create(
            messages=[{"role": "user", "content": f"Проведи технический аудит: {content}"}],
