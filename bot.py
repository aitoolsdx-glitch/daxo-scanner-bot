...
# Импортируем Dispatcher напрямую из его папки, чтобы не было ошибок
...
async def analyze(message: types.Message):
    # Если это не команда, то анализируем
    content = ""
    if message.document:
    ...
    try:
        completion = client.chat.completions.create(
            messages=[{"role": "user", "content": f"Сделай краткий отчет: {content}"}],
