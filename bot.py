...
async def cmd_start(message: types.Message):
    ...
    await message.answer(">> CHIIP System Online\nОтправь файл или текст для анализа безопасности.")
...
async def call_broadcast(callback: types.CallbackQuery):
    await callback.message.answer("Используй команду: `/send ВашТекст` для рассылки всем юзерам.")
    await callback.answer()
...
async def cmd_send_all(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        if not text: return await message.answer("Ошибка: напишите текст.")
        count = 0
        for u in users_db:
            try:
...
async def analyze(message: types.Message):
    # Добавляем в базу всех, кто пишет
    ...
    if message.document:
        ...
        content = "Данные без описания"

    status = await message.answer("🔍 **CHIIP анализирует структуру...**")
    try:
        completion = client.chat.completions.create(
            messages=[{"role": "user", "content": f"Сделай краткий отчет по безопасности объекта: {content}"}],
